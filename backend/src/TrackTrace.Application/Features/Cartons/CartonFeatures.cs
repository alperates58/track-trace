    ...
    IRequestHandler<AddProductToCartonCommand, Unit>
{
    ...
    public async Task<(IEnumerable<CartonDto> Items, int TotalCount)> Handle(GetCartonsQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            string cleanSearch = ExtractSscc(request.Search);
            builder.Where("(c.CartonNo ILIKE @Search OR c.SSCC ILIKE @Search OR o.OrderNo ILIKE @Search OR c.SSCC = @CleanSearch)", 
                new { Search = $"%{request.Search}%", CleanSearch = cleanSearch });
        }
